...
class Loja(commands.Cog):
    ...
    def remove_almas(self, user_id: str, quantidade: int) -> bool:
        ...
        try:
            ...
            if soul_atual >= quantidade:
                ...
                # Adicionar ao contador de souls gastos na economia (bot_souls)
                bot_souls = db.get("bot_souls", 0)
                db["bot_souls"] = bot_souls + quantidade
                self.bot.save_db(db)
                return True
            return False
        except:
    ...
    async def autocomplete_item_comprar(self, interaction: discord.Interaction, current: str):
        ...
        try:
            ...
            print(f"Erro no autocomplete_item_comprar: {e}")
            return [app_commands.Choice(name="❌ Erro ao carregar itens", value="erro")]
    ...
    async def autocomplete_box(self, interaction: discord.Interaction, current: str):
        ...
        try:
            ...
            user_inv = self.get_user_inventory(user_id_str)
            
            if not user_inv or not isinstance(user_inv, dict):
                return [app_commands.Choice(name="❌ Erro ao carregar inventário", value="erro")]
            
            itens_inv = user_inv.get("itens", {})
            if not isinstance(itens_inv, dict):
                itens_inv = {}
